...
# @@License          :  WTFPL
# @@ReadME           :  zz-mongo-express.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Tuesday, Aug 22, 2023 23:54 EDT
# @@File             :  zz-mongo-express.sh
# @@Description      :
...
DATABASE_DIR="${DATABASE_DIR_SERVICE_SAMPLE:-/data/db/mongo-express}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set webroot
WWW_DIR="/usr/share/webapps/mongodn"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
ETC_DIR="/etc/mongo-express"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
RUN_DIR="/run/init.d"              # set scripts pid dir
LOG_DIR="/data/logs/mongo-express" # set log directory
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the working dir
...
root_user_name="${SERVICE_SAMPLE_ROOT_USER_NAME:-}" # root user name
root_user_pass="${SERVICE_SAMPLE_ROOT_PASS_WORD:-}" # root user password
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
user_name="${SERVICE_SAMPLE_USER_NAME:-}"      # normal user name
user_pass="${SERVICE_SAMPLE_USER_PASS_WORD:-}" # normal user password
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Overwrite variables from files
__file_exists_with_content "${USER_FILE_PREFIX}/mongodb_name" && user_name="$(<"${USER_FILE_PREFIX}/mongodb_name")"
__file_exists_with_content "${USER_FILE_PREFIX}/mongodb_pass" && user_pass="$(<"${USER_FILE_PREFIX}/mongodb_pass")"
__file_exists_with_content "${ROOT_FILE_PREFIX}/mongodb_name" && root_user_name="$(<"${ROOT_FILE_PREFIX}/mongodb_name")"
__file_exists_with_content "${ROOT_FILE_PREFIX}/mongodb_pass" && root_user_pass="$(<"${ROOT_FILE_PREFIX}/mongodb_pass")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
EXEC_CMD_BIN="npm"        # command to execute
EXEC_CMD_ARGS="run start" # command arguments
...
SERVICE_PORT="${VCAP_APP_PORT:-${SERVICE_PORT:-80}}"
VCAP_APP_HOST="${VCAP_APP_HOST:-0.0.0.0}"
ME_CONFIG_OPTIONS_EDITORTHEME="${ME_CONFIG_EDITORTHEME:-dracula}"
ME_CONFIG_HEALTH_CHECK_PATH="${ME_CONFIG_HEALTH_CHECK_PATH:-/health}"
ME_CONFIG_SITE_GRIDFS_ENABLED="${ME_CONFIG_SITE_GRIDFS_ENABLED:-true}"
ME_CONFIG_MONGODB_ENABLE_ADMIN="${ME_CONFIG_MONGODB_ENABLE_ADMIN:-true}"
ME_CONFIG_MONGODB_URL="${ME_CONFIG_MONGODB_URL:-mongodb://$HOSTNAME:27017/?ssl=false}"
ME_CONFIG_BASICAUTH_USERNAME="${ME_CONFIG_BASICAUTH_USERNAME:-}"
ME_CONFIG_SITE_COOKIESECRET="${ME_CONFIG_SITE_COOKIESECRET:-$(__random_password 12)}"
ME_CONFIG_SITE_SESSIONSECRET="${ME_CONFIG_SITE_SESSIONSECRET:-$(__random_password 10)}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Specifiy custom directories to be created
ADD_APPLICATION_FILES=""
ADD_APPLICATION_DIRS=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
APPLICATION_FILES="$LOG_DIR/mongo-express.log"
...
CMD_ENV='PORT="'${SERVICE_PORT:-80}'" ME_CONFIG_HEALTH_CHECK_PATH="'${ME_CONFIG_HEALTH_CHECK_PATH:-}'" ME_CONFIG_BASICAUTH_USERNAME="'${ME_CONFIG_BASICAUTH_USERNAME:-}'" ME_CONFIG_OPTIONS_EDITORTHEME="'${ME_CONFIG_OPTIONS_EDITORTHEME:-}'" VCAP_APP_HOST="'${VCAP_APP_HOST}'" ME_CONFIG_MONGODB_URL="'${ME_CONFIG_MONGODB_URL:-}'" ME_CONFIG_MONGODB_ENABLE_ADMIN="'${ME_CONFIG_MONGODB_ENABLE_ADMIN}'"'
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  if [ ! -f "/config/mongo-express/.env" ]; then
    cat <<EOF >"/config/mongo-express/.env"
...
EOF
  ...
  # __file_copy "$CONF_DIR/." "$WWW_DIR/" |& tee -a "$LOG_DIR/init.txt" &>/dev/null
  # replace variables
  # __replace "" "" "$ETC_DIR/mongo-express.conf"
  # replace variables recursively
  # __find_replace "" "" "$ETC_DIR/"
  # replace defaults in ETC_DIR
  # __initialize_replace_variables "$WWW_DIR"
  # custom commands
  [ -f "/config/mongo-express/.env" ] && ln -sf "/config/mongo-express/.env" "$WWW_DIR/.env"
  [ -f "/config/mongo-express/config.js" ] && ln -sf "/config/mongo-express/config.js" "$WWW_DIR/config.js"
